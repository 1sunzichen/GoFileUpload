...
func CompleteUploadHandler(c *gin.Context) {
	...
	ferr := dbplayer.OnFileUploadFinished(fmeta.FileSha1,fmeta.FileName,fmeta.FileSize,fmeta.Location)
	uferr := dbplayer.OnUserFileUploadFinished(username, filehash,filename,fmeta.FileSize)
	if !ferr || !uferr  {
		log.Println(err)
		c.JSON(
			http.StatusOK,
